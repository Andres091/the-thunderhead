...
module.exports.run = async (client, message, args) => {
  ...
  var url = "https://www.youtube.com/watch?v=8GW6sLrK40k&list=PL3ACSjctShvtgX5uOe0vLLgr0tPTgwnPe&index"
  const playlist = await ytpl(url.split("list=")[1])
  const videos = playlist.items;
  message.channel.send(client.msg["music_playlist_success"].replace("[PLAYLIST_TITLE]", `${playlist.title} (${videos.length})`))
  for (const video of videos) await queueSong(video, message, message.member.voice.channel, client.queue)
}



module.exports.config = {
  name: "scythecommunityplaylist",
  aliases: [],
  use: "scythecommunityplaylist",
  description: "Plays the `Scythe Community Playlist`",
  state : "gamma",
  page: -1
};
